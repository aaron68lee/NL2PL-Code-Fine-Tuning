    ...
    if args.do_test:
        ...
        for idx,file in enumerate(files):   
            ...
            for batch in tqdm(eval_dataloader,total=len(eval_dataloader)):
                ...
                with torch.no_grad():
                    ...
                    for pred in preds:
                        ...
                        text = tokenizer.decode(t,clean_up_tokenization_spaces=False)
                        p.append(text)
                # break # run only one batch for time saving
            model.train()
            predictions=[]
            with open(os.path.join(args.output_dir,"test_{}.output".format(str(idx))),'w') as f, \
                    ...
                    open(os.path.join(args.output_dir,"test_{}.source".format(str(idx))),'w') as f2:
                for ref,gold in zip(p,eval_examples):
                    predictions.append(str(gold.idx)+'\t'+ref)
                    f.write(str(gold.idx)+'\t'+ref+'\n')
                    f1.write(str(gold.idx)+'\t'+gold.target+'\n')  
                    f2.write(str(gold.idx)+'\t'+gold.source+'\n')  
